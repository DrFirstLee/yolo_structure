...
test_video_file = rtsp_url # os.path.join(video_path,"korea_long.mp4")
FRAME_MAX_NUM = 1800
...
if MAKE_VIDEO == False:
    ...
    output_path = f"{results_dir}/{video_unique}_{current_time}.avi"
else:
    final_file_name = f"{video_unique}_video_{current_time}.csv"
    status_csv = os.path.join(results_dir,final_file_name)
    output_path = f"{results_dir}/{video_unique}_video_{current_time}.avi"
...
# 총 프레임 수 가져오기
total_frames = FRAME_MAX_NUM # int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
...
print(f"VIDEO SETTING // frame_width : {width} , frame_height : {height}, fps : {fps}")
